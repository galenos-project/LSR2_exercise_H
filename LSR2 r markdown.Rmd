...
-   Exercise augmented by treatment as usual (TAU)/therapy (exercise only vs exercise + therapy/TAU)
...
There was no significant difference in PTSD symptom severity between studies with moderate intensity and those with high intensity exercise. However, visual inspection of the forest plot suggests the effect is larger in the high intensity exercise subgroup.
...
There was no significant difference in PTSD symptom severity between subgroups of studies according to the type of exercise (aerobic, anaerobic, or mixed).

##### Exercise alone or TAU/therapy augmented by exercise
...
Overall, there is no strong evidence that the intervention length affected the treatment effect. The meta-regression analysis yielded a coefficient of `r round(intervLENGTHreg$beta[2], 2)` with a 95% CI ranging from `r round(intervLENGTHreg$ci.lb[2], 2)` to `r round(intervLENGTHreg$ci.ub[2], 2)`.
...
`r length(table(data_2023_12_4_LSR2_Awks2$studyN))` studies provided data for treatment dropout, and contributed `r length(data_2023_12_4_LSR2_Awks2$tg1Ndrop)` effect measures to the treatment dropout meta-analysis. The forest plot for the risk of treatment dropout is shown in Figure 7.
...
When restricting the analysis to studies with moderate or low risk of bias, the effect of exercise on PTSD symptoms severity is `r paste(rob.sens.MA$sm, "=", round(rob.sens.MA$TE.random, 2),", (95% CI", round(rob.sens.MA$lower.random, 2), "to", round(rob.sens.MA$upper.random, 2))`). For reference, the main effect size for the primary outcome is `r paste(LSR2mainMA$sm, "=", round(LSR2mainMA$TE.random, 2),", 95% CI", round(LSR2mainMA$lower.random, 2), "to", round(LSR2mainMA$upper.random, 2))`), so the results do not change substantially.

### Reporting bias

The forest plot below show the meta-analysis results of the primary outcome ordered by precision of the studies. It seems that the smaller studies showed larger effects favouring the intervention.
